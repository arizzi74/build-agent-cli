...
The OAuth access token is sent in the Nirvana websocket handshake payload under the Build Agent parameters. Tokens are cached locally in `oauth-token.json`; the CLI does not use macOS Keychain or browser automation for the token cache. Default Nirvana startup is session-first: it validates/reuses matching `session.json`, otherwise uses the existing form login and persists that session, then requests `/oauth_auth.do` with the authenticated session and PKCE. Returned authorization codes are accepted from JSON/XML, redirects, final URL queries, or redirect responses with state validation. Explicit consent requires an interactive allow confirmation; it is never auto-approved. If session OAuth cannot finish, the CLI uses the manual PKCE fallback; it prints the authorization URL only when the browser cannot be opened or `--no-open` is used, and that URL has only transient PKCE/state parameters (never credentials, cookies, codes, or tokens). After validation it reads `glide.ui.session_timeout`; ServiceNow guidance favors <=60 minutes (fallback 30), while Antonio's requested 1440-minute/24-hour convenience setting is materially weaker. A non-1440 or absent value is only updated/created after explicit interactive confirmation and reread verification, never under `--auto-approve`. Permission failures warn and OAuth continues. `--auth basic` remains web-gateway-only because it cannot support session-derived Nirvana OAuth.
...
- `/api/sn_build_agent/build_agent_api/conversations` browser-HAR parity: list first through `?application_id_list=<appSysIds>&client=ide`, create payload `title/applicationId/applicationName/client`, adopt returned `sysId`, load selected messages first through `GET /conversations/<conversationId>/messages` (including nested `content` role/text objects and ordered `sequence` rows), and message payload `content` only; old `conversations_api` and table reads stay compatibility fallbacks only for missing/unsupported routes, never to mask auth/server failures
