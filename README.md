...
Use `/sync` to merge local source and Glider files. `/sync status` reports pending local/remote changes, `/sync pull` accepts remote-only changes, and `/sync push` accepts local-only changes. When no checkout exists, `/sync status` compares the remote tree with an empty baseline and reports the expected local path without creating the app directory, registry, sync metadata, or project lock. A later pull can safely adopt the exact lock-only residue left by older status versions, while symlinks, foreign manifests, user files, and unexpected metadata remain collisions. Bare `/sync` performs the safe bidirectional merge but never uploads local edits; `/sync push` is the sole upload path. If both sides changed a path after the recorded baseline, it stops and reports a conflict rather than overwriting either side. It deliberately excludes `node_modules`, `dist`, `target`, `.git`, `.jest_cache`, local caches, sync metadata, and the root `package-lock.json`. The lockfile is treated as derived local dependency state: bacli runs npm with lockfile writes disabled, never uploads it implicitly, and does not let an older locally generated or remote lockfile trap build preparation in a sync-conflict loop. Build/dependency preparation holds the checkout lock through sync, dependency installation, build, generated-file persistence, and packing: clean or remote-only checkouts proceed, while pending local source edits require `/sync push` and divergence blocks before npm/build. If a build/dependency operation encounters an ambiguous non-symlink canonical directory, it first compares bounded local/Web state and asks for an explicit replacement confirmation (never auto-approved); approval retains the old directory as a sibling backup and rehydrates a fresh v2 checkout strictly from Web contents. Once a local source edit is explicitly pushed, it is present in the Glider workspace and can be built from the ServiceNow Web UI.
