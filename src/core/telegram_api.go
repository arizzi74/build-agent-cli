...
// sendPreformattedText renders a bounded status message in Telegram's native
// fixed-width HTML block. Telegram does not support arbitrary text colours;
// callers use Unicode status markers for the equivalent visual cue. All
// dynamic text is escaped before it is passed to Telegram's HTML parser.
func (api *telegramAPI) sendPreformattedText(ctx context.Context, chatID, text string) error {
	text = telegramSafeRemoteText(text, 0)
	if text == "" {
		text = "(no output)"
	}
	formatted := "<pre>" + html.EscapeString(text) + "</pre>"
	if telegramUTF16Units([]rune(formatted)) > telegramTextChunkUTF16Units {
		// A preformatted block cannot be safely split without reparsing its
		// markup. Preserve the complete diagnostic as plain text instead.
		return api.sendText(ctx, chatID, text)
	}
	return api.call(ctx, "sendMessage", map[string]interface{}{
		"chat_id": chatID, "text": formatted, "parse_mode": "HTML", "disable_web_page_preview": true,
	}, nil)
}
